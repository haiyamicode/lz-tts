...
class VitsModel(pl.LightningModule):
    ...
    def training_step_g(self, batch: Batch):
        ...
        bert_input = None
        if getattr(self.hparams, "use_bert", False) and batch.texts is not None:
            ...
            bert_input = build_bert_input(batch.texts, self._semantic_tokenizer)

            if bert_input is not None:
                # Move semantic inputs to the same device as the phoneme ids / model.
                dev = x.device
                bert_input = {
                    "input_ids": bert_input["input_ids"].to(dev),
                    "attention_mask": bert_input["attention_mask"].to(dev),
                }
    ...
    def add_model_specific_args(parent_parser):
        ...
        parser.add_argument(
            ...
            type=float,
            default=0.5,
            help="Weight for semantic features in fusion layer (0=phoneme-only, 1=semantic-only)",
        )
        #
