...
class TextEncoder(nn.Module):
    def __init__(
        ...
        self.duration_blend_sdp_ratio = float(getattr(duration_predictor_params, "sdp_ratio", 0.2))
        self.duration_sdp_noise_scale = float(getattr(duration_predictor_params, "noise_scale_w", 0.8))

        self.emb = torch.nn.Embedding(n_vocab, self.n_channels)
        torch.nn.init.normal_(self.emb.weight, 0.0, self.n_channels**-0.5)
        if self.semantic_dim > 0:
            self.semantic_projection = torch.nn.Linear(self.semantic_dim, self.n_channels)
            self.semantic_layer_norm = torch.nn.LayerNorm(self.n_channels)
        else:
            self.semantic_projection = None
            self.semantic_layer_norm = None
    ...
    def forward(self, x, x_lengths, spks=None, semantic_features=None, return_duration_hidden=False):
        ...
        if semantic_features is not None:
            ...
            if semantic_features.dim() != 3:
                raise ValueError(f"semantic_features must be 3D, got {tuple(semantic_features.shape)}")
            if semantic_features.size(1) == self.semantic_dim:
                semantic_features = semantic_features.transpose(1, 2)
            elif semantic_features.size(2) != self.semantic_dim:
                raise ValueError(
                    f"semantic feature dim mismatch: expected {self.semantic_dim}, "
                    f"got {tuple(semantic_features.shape)}"
                )

            if semantic_features.size(1) < x.size(1):
                semantic_features = torch.nn.functional.pad(
                    semantic_features,
                    (0, 0, 0, x.size(1) - semantic_features.size(1)),
                )
            elif semantic_features.size(1) > x.size(1):
                semantic_features = semantic_features[:, : x.size(1)]
            x = self.semantic_layer_norm(x + self.semantic_projection(semantic_features.to(dtype=x.dtype)))
    ...
    def blend_duration_logw(
        ...
        if self.proj_w_sdp is None:
            return logw_dp
        ratio = self.duration_blend_sdp_ratio if sdp_ratio is None else float(sdp_ratio)
        if ratio <= 0.0:
            return logw_dp
        ratio = min(ratio, 1.0)
