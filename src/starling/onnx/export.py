...
from src.starling.cli import VOCODER_URLS, load_matcha, load_vocoder
...
def get_exportable_module(matcha, vocoder, n_timesteps):
    ...
    def onnx_forward_func(x, x_lengths, scales, spks=None):
        ...
        output = src.starling.synthesise(x, x_lengths, n_timesteps, temperature, spks, length_scale)
        return output["mel"], output["mel_lengths"]

    # Monkey-patch Matcha's forward function
    src.starling.forward = onnx_forward_func
...
def main():
    ...
    is_multi_speaker = src.starling.n_spks > 1
