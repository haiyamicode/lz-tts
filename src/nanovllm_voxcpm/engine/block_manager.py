"""src.nanovllm_voxcpm.engine.block_manager
...
--------------------------------
:class:`~src.nanovllm_voxcpm.engine.scheduler.Scheduler` is the only policy layer.
...
``src.nanovllm_voxcpm/models/voxcpm/engine.py``, the sequence's ``token_ids``
...
class BlockManager:
    ...
    def allocate(self, seq: Sequence):
        ...
        h = self._initial_prefix_hash(seq)
        cache_miss = False
        ...
        if len(seq) % self.block_size == 1:
            ...
            if last_block.hash == -1:
                ...
                prefix = (
                    ...
                    else self._initial_prefix_hash(seq)
                )
