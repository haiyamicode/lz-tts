"""src.nanovllm_voxcpm.engine.sequence
...
---------------------------
The engine operates in a tight loop (see :class:`~src.nanovllm_voxcpm.engine.llm_engine.LLMEngineBase`):

1) Users (or model-specific engines) construct a :class:`Sequence` from an input
   prompt and enqueue it into :class:`~src.nanovllm_voxcpm.engine.scheduler.Scheduler`.
2) The :class:`~src.nanovllm_voxcpm.engine.scheduler.Scheduler` transitions sequences
   between ``WAITING`` and ``RUNNING`` and decides which ones to execute next.
3) :class:`~src.nanovllm_voxcpm.engine.block_manager.BlockManager` allocates/free KV
   blocks and fills ``Sequence.block_table`` (physical block ids used by the
   Attention KV-cache).
4) The model-specific engine converts :class:`Sequence` ->
   :class:`~src.nanovllm_voxcpm.engine.model_runner.RunnerTask` (runner-friendly
...
- ``num_cached_tokens``: Count of prompt tokens whose KV state is reused from
  the prefix cache (see :mod:`src.nanovllm_voxcpm.engine.block_manager`).
...
------------------------
The VoxCPM implementation under ``src.nanovllm_voxcpm/models/voxcpm`` uses
