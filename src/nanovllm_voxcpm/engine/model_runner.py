"""src.nanovllm_voxcpm.engine.model_runner

This module defines the GPU execution abstraction used by the engine.

The high-level runtime separates concerns:
- :mod:`src.nanovllm_voxcpm.engine.scheduler` decides *what to run* (which sequences)
  and manages KV-cache block allocation.
- :mod:`src.nanovllm_voxcpm.engine.llm_engine` orchestrates the step loop and
...
tensors stored inside causal :class:`~src.nanovllm_voxcpm.layers.attention.Attention`
...
- Prepare attention metadata ("context") for flash-attn kernels via
  :func:`src.nanovllm_voxcpm.utils.context.set_context`.
...
------------------------
``src.nanovllm_voxcpm/models/voxcpm/runner.py`` shows a typical implementation:
...
from src.nanovllm_voxcpm.engine.lora_manager import (
    LoRAModelPayload,
    LoRARuntime,
